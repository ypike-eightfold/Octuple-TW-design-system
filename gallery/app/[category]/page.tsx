...
export default async function CategoryPage({ params }: { params: Params }) {
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-[var(--color-muted)]">{meta.blurb}</p>

      {designs.length === 0 ? (
        <div className="mt-10 rounded-lg border border-dashed border-[var(--color-border)] p-10 text-center text-sm text-[var(--color-muted)]">
        ...
        <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {designs.map((d) => (
            <Link
              ...
              className="group overflow-hidden rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] transition hover:border-[var(--color-accent)]"
            >
              ...
              <div className="p-4">
                <div className="flex items-center justify-between gap-2">
                  <h2 className="font-medium group-hover:text-[var(--color-accent)]">{d.title}</h2>
                  <span className="shrink-0 rounded-full border border-[var(--color-border)] px-2 py-0.5 text-[10px] uppercase tracking-wide text-[var(--color-muted)]">
                    {d.version}
                  </span>
                </div>
                <p className="mt-1.5 line-clamp-2 text-sm text-[var(--color-muted)]">{d.description}</p>
              </div>
            </Link>
          ))}
        </div>
