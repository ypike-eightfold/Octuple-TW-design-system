  ...
  return (
    <div>
      <nav className="mb-6 text-sm text-[var(--color-muted)]">
        ...
        <Link href={`/${category}`} className="hover:underline">{categoryMeta.name}</Link>
        <span aria-hidden> / </span>
      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-[var(--color-muted)]">{design.description}</p>
        </div>
        <span className="shrink-0 rounded-full border border-[var(--color-border)] px-3 py-1 text-xs uppercase tracking-wide text-[var(--color-muted)]">
      ...
      <section className="mt-8">
        <h2 className="mb-3 text-sm font-medium text-[var(--color-muted)]">Prototype</h2>
      ...
      <section className="mt-8">
        <h2 className="mb-3 text-sm font-medium text-[var(--color-muted)]">Thumbnail</h2>
        <img
